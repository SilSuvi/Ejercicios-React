  ...
  let videoclub = {
    peliculas: [
      ...
      },
      {
        ...
        cartel: "./img/beware.jpg",
      },
    ],
  };

  const showFilms = videoclub.peliculas.map(function (film) {
    return (
      <div className="card" key={film.titulo}>
        <img className="cartel" src={film.cartel} alt="cartel" />
        <h3 className="info">{film.titulo}</h3>
        <p className="info">{film.sinopsis}</p>
      </div>
    );
  });

  return <div className="contenedor">{showFilms}</div>;
}
